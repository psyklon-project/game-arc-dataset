...
| [007](./007.json) | Match 3              | Delete each sign except the ones where 3 are is in a row, column, or diagonal line. |
| [008](./008.json) | Math                 | Add or subtract the number of dots and put your answer with red in the place of the gray are, following the pattern of the others. |
| [009](./009.json) | Spot The Difference  | Compare the imags. Leave only the right one, and recolor every object to gray that chanegd position. |
...
| [025](./025.json) | Connect Colors       | Connect the squares with the same color, witout lines crossing a gray area or each other. Always use the shortest path and turn as few times as possible, but follow the grid. |
...
| [033](./033.json) | Lemmings             | The Lemmings are coming from the red pixel and start moving to the right until they hit something, then they change direction. If they fall down and hit the gray line they die. Place your special Lemmings from the bottom row to the playfield to help the others reach the green goal. LIGHTBLUE must be placed right before an edge to give parachute to other Lemmings so they can survive the fall. BROWN acts like a wall, hitting it from the side will reverse the direcgtion, but it can act as walking ground too. YELLOW will dig a hole underneath himself, and PINK will make a hole in the horizontal movement direction. Highlight the path of from start to finish with red, including removed ground. |
